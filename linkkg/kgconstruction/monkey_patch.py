...
def _load_graphrag_symbol(symbol_name, candidates):
    ...
    for module_info in pkgutil.walk_packages(graphrag.__path__, graphrag.__name__ + "."):
        try:
    ...
    raise ModuleNotFoundError(f"Unable to locate {symbol_name} in installed graphrag package")


def patch_openai_embeddings_llm():
    OpenAIEmbeddingsLLM = _load_graphrag_symbol(
        "OpenAIEmbeddingsLLM",
        [
            "graphrag.index.llm.openai.openai_embeddings_llm",
...
def patch_query_embedding():
    OpenAIEmbedding = _load_graphrag_symbol(
        "OpenAIEmbedding",
...
def patch_global_search():
    ...
    GlobalSearch = _load_graphrag_symbol(
        "GlobalSearch",
        ["graphrag.query.structured_search.global_search.search"],
    )
    num_tokens = _load_graphrag_symbol(
        "num_tokens",
        ["graphrag.query.llm.text_utils"],
    )
    SearchResult = _load_graphrag_symbol(
        "SearchResult",
